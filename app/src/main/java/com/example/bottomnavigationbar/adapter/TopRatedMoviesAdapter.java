...
public class TopRatedMoviesAdapter extends RecyclerView.Adapter<TopRatedMoviesAdapter.MovieViewHolder> {
    ...
    public TopRatedMoviesAdapter(ArrayList<Movie> movies, Context context) {
        mMovies = movies;
        mContext = context;
    }
    ...
    @NonNull
    @Override
    public TopRatedMoviesAdapter.MovieViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
            View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.movie_item_view, parent, false);
            return new MovieViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull TopRatedMoviesAdapter.MovieViewHolder holder, int position) {
            Glide.with(mContext).load(Constants.IMAGE_BASE_URL + mMovies.get(position).getPosterPath())
                    .into(holder.mMovieAvatar);
